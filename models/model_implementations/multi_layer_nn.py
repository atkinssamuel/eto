...
class NN(Model):
    ...
    def init_layers(self, X):
        self.layers.append(LinearLayer(input_dim=X.shape[1], output_dim=X.shape[1] * 2, lr=self.lr, mu=self.mu))
        self.layers.append(SigmoidLayer())
        self.layers.append(LinearLayer(input_dim=X.shape[1] * 2, output_dim=X.shape[1], lr=self.lr, mu=self.mu))
        self.layers.append(SigmoidLayer())
